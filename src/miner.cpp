...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("shmnMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("shmnMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("shmnMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shmn-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running shmnMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
