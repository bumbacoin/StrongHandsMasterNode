...
#if defined(NDEBUG)
#error "shmn cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "shmn Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 10) {
        nSubsidy = 380000 * COIN;		// 4.18 millions
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 10) {
        nSubsidy = 0 * COIN;
    } else if (nHeight <= 13440 && nHeight > Params().LAST_POW_BLOCK()) {		// approx 2-3 week period
        nSubsidy = 1 * COIN;
    } else if (nHeight <= 26880 && nHeight > 13440) {		// approx 4 week period
        nSubsidy = 1.5 * COIN;
    } else if (nHeight <= 53760 && nHeight > 26880) {		// approx 8 week period
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 5 * COIN;
    }
